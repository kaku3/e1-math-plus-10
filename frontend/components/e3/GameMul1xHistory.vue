      ...
      <v-tab-item class="green lighten-5">
        ...
        <v-card-text>
          <v-row class="mt-2 mb-2">
            ...
            <v-spacer></v-spacer>
            <v-col cols="auto">
              <v-btn @click="startGame('mul1xSprint-10')" color="primary"><v-icon>mdi-play</v-icon>はじめる</v-btn>
            </v-col>
          </v-row>
        </v-card-text>
        <Ranking gameMode="mul1xSprint" :questionCount="10" />
      </v-tab-item>
      <v-tab-item class="green lighten-5">
        ...
        <v-card-text>
          <v-row class="mt-2 mb-2">
            ...
            <v-spacer></v-spacer>
            <v-col cols="auto">
              <v-btn @click="startGame('mul1xSprint-30')" color="primary"><v-icon>mdi-play</v-icon>はじめる</v-btn>
            </v-col>
          </v-row>
        </v-card-text>
        <Ranking gameMode="mul1xSprint" :questionCount="30" />
      </v-tab-item>
      <v-tab-item class="green lighten-5">
        ...
        <v-card-text>
          <v-row class="mt-2 mb-2">
            ...
            <v-spacer></v-spacer>
            <v-col cols="auto">
              <v-btn @click="startGame('mul1xEndress')" color="primary"><v-icon>mdi-play</v-icon>はじめる</v-btn>
            </v-col>
          </v-row>
        </v-card-text>
        <Ranking gameMode="mul1xEndress" :questionCount="-1" />
      </v-tab-item>
    </v-tabs>
  </v-card>
</template>
...
export default Vue.extend({
  ...
  computed: {
    ...
    sprint10Scores(): ScoreEntity[] {
      return GameHistoryUtil.termFilteredScoreEntities('mul1xSprint-10', this.scores, this.historyDates)
    },
    sprint30Scores(): ScoreEntity[] {
      return GameHistoryUtil.termFilteredScoreEntities('mul1xSprint-30', this.scores, this.historyDates)
    },
    endressScores(): ScoreEntity[] {
      return GameHistoryUtil.termFilteredScoreEntities('mul1xEndress', this.scores, this.historyDates)
    },
