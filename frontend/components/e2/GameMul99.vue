<template>
  <div class="game">
    ...
    <v-card>
      ...
      <v-card-text v-if="isEnd" class="blue lighten-5">
        ...
        <v-row class="ex-canvas text-center">
          <v-col cols="12" align-self="center">
            <v-btn
              ...
              @click="goGameIndex()"
            >
              <v-icon>mdi-playlist-edit</v-icon>かけざん９９
            </v-btn>
            <v-btn
              x-large
              color="blue"
...
export default Vue.extend({
  ...
  methods: {
    ...
    next() {
      ...
      this.answer = 0
      this.question1 = Math.floor(Math.random() * 9) + 1
      this.question2 = this.question1 * (Math.floor(Math.random() * 9) + 1)
    ...
    onAnswer(v: number) {
      ...
      if(this.question1 * v === this.question2) {
        this.score += 1
        if(this.gameMode === 'mul99Endress') {
  ...
  computed: {
    ...
    isSprint(): boolean {
      return this.gameMode === 'mul99Sprint'
    },
    isEndress(): boolean {
      return this.gameMode === 'mul99Endress'
    },
    displayScore(): number {
      return this.gameMode === 'mul99Sprint' ? this.score : this.score2
    },
